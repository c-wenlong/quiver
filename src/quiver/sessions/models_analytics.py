            ...
            with os.scandir(claude_dir) as entry_it:
                for entry in entry_it:
                    if not entry.is_dir() or not entry.name.startswith("-"):
                        continue
                    # ⚡ Bolt: Use os.scandir to avoid redundant stats from glob.glob
                    try:
                        with os.scandir(entry.path) as jsonl_it:
                            for j_entry in jsonl_it:
                                if j_entry.is_file() and j_entry.name.endswith(".jsonl"):
                                    for key, cnt in _scan_jsonl_models(j_entry.path, 30).items():
