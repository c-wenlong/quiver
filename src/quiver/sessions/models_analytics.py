...
def collect_model_usage() -> dict[str, dict[tuple[str, str], int]]:
    ...
    if os.path.exists(freebuff_dir):
        try:
            seen: dict[tuple[str, str], int] = {}
            for proj_entry in os.scandir(freebuff_dir):
                if not proj_entry.is_dir():
                    continue
                project = proj_entry.name
                project_path = proj_entry.path
                chats_dir = os.path.join(project_path, "chats")
                if not os.path.exists(chats_dir):
                    continue
                for sess_entry in os.scandir(chats_dir):
                    if not sess_entry.is_dir():
                        continue
                    session_dir = sess_entry.name
                    session_path = sess_entry.path
