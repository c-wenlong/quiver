        ...
        try:
            ...
            with os.scandir(sess_dir) as name_entry_it:
                for name_entry in name_entry_it:
                    try:
                        mt = name_entry.stat().st_mtime * 1000
                    except Exception:
                        mt = 0.0
                    ts = max(ts, mt)
        except Exception:
        ...
        try:
            ...
            with os.scandir(brain_dir) as d_entry_it:
                for d_entry in d_entry_it:
                    ...
                    title = ""
                    try:
                        with os.scandir(d_entry.path) as md_entry_it:
                            for md_entry in md_entry_it:
                                if md_entry.name.endswith(".metadata.json"):
                                    try:
                                        mt = md_entry.stat().st_mtime * 1000
                                    except Exception:
                                        mt = 0.0
                                    if mt > mtime:
                                        mtime = mt
                                        try:
                                            with open(md_entry.path) as f:
                                                data = json.load(f)
                    ...
                    path = ""
                    overview_path = os.path.join(d_entry.path, ".system_generated", "logs", "overview.txt")
