...
`tests/test_syntax_floor.py` compiles every source file with the oldest interpreter it can find. Do not use PEP 701 nested f-strings (quotes reused inside a replacement field); they pass on 3.12 and crash on import for 3.10 users. This shipped as a real bug in 0.2.8.
...
| `init/` | writes the shared `AGENTS.md` and skills tree, symlinks them into each harness, migrates the old `~/.config/swe` root |
