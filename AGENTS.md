...
For `swe init`, `archived` also means unmanaged: a path of an archived harness that would change is reported `ignored` ("archived in harness.json"), while an already-`linked` path stays linked — init never unlinks. Known harnesses are declared in `init/layout.py`'s `HARNESS_SIGNATURES`: skills and instruction targets plus `evidence`, an any-of list of home-relative paths that mean "installed" (the targets' own parents count implicitly). Evidence exists because it can live away from the targets — kilo proves itself with `~/.config/kilo` while its skills root is `~/.kilo/skills`. Registry `capabilities.instructions.file` / `capabilities.skills.root` entries always override or extend signatures. A discovered skills root that no registry entry, alias or signature claims triggers the init picker (`--yes` registers all without asking): ticked harnesses are registered and linked, unticked ones are archived and left alone.
...
- **New table:** use `table.py` (`Table`, column kinds `text`, `number`, `count_threshold`, `list`, `timestamp`, `preformatted`; custom kinds via `@register_kind`). Pre-compute rows outside the loop, pin columns once, `add_row` per item, `render` once. Do not hand-roll f-string padding — `preformatted` cells carry their own ANSI and the table pads them to the column width (`right=True` right-aligns); `trust_cell_width=True` is the verbatim opt-out for cells that pre-pad themselves.
...
- **Title provenance** (`Session.title_source`): "" = first user prompt, "rename" = user-set name, "auto" = harness-generated. `swe session` renders "rename" bright italic and everything else dim. Only harnesses with an on-disk discriminator ever emit "rename": Claude (`custom-title` event in the transcript tail), Copilot (`user_named:` in `session-state/<id>/workspace.yaml`), Grok (`summary.json` `title_is_manual`), Droid (`session_start.isSessionTitleManuallySet`), Pi (`session_info` event), Devin (an inline `/rename <name>` or `/rename-chat <name>` in `prompt_history` whose argument equals the title; a name typed into the interactive rename prompt is never recorded, so that session reads as auto), Codex (inferred, see below). opencode, Forge, Continue and Cursor write a rename into the same field as their auto-titler with no marker, so they can never be told apart; do not invent heuristics for them. Cline, Freebuff and Gemini have no rename at all.
...
- **Tool identity** (`sessions/identity.py`): `LAUNCH_TOOL` maps session `tool_name` to the CLI binary for resume; `COUNT_TO_REGISTRY` maps to the registry key for `swe list` counts. Both are empty today: every parser's `tool_name` is already its launch binary and registry key.
...
- **Session status** (`sessions/status.py`): the `swe session` `ST` column resolves one label per row (`active`, `done`, `followup`, `error`, `interrupted`, or "" for harnesses with no probe) and `sessions/commands.py::_STATUS_GLYPHS` maps each to a single-width glyph, colour and legend label; real emoji are two columns wide and `console.visible_len` counts code points, so they would break the grid. Six per-harness signals feed it. Claude: the transcript tail's decisive record, plus the `~/.claude/sessions/*.json` live-pid registry and `~/.claude/jobs/*/state.json` (blocked → followup, failed → error). Cursor: the terminal `turn_ended` record (`success`, `error`, `aborted`). Devin: the `main_chain_id` head row of `message_nodes` (node_id is not unique across sessions, always filter by session_id), plus the `~/.local/share/devin/cli/session_locks/<id>.lock` pid files `devin acp` leaves while it runs. Codex: the rollout tail's decisive `event_msg` (`task_complete` → finished with `last_agent_message`, `turn_aborted` → aborted, `task_started` → midturn; the rest are bookkeeping), falling back to the last `response_item` when no decisive event is in the window; the rollout file is found at `~/.codex/sessions/Y/M/D/<stem>.jsonl` by parsing the date out of the stem, with a `*/*/*/` glob as fallback, and codex records no error signal. OpenCode: the session's last `message` row in `~/.local/share/opencode/opencode.db` — a `user` role or an assistant `finish` of `tool-calls`/absent is mid-turn, `error` is error, any other finish value closes the turn with text taken from that message's own `part` rows. Pi: the trailing `message` record in the JSONL transcript `session_id` literally points at — a `toolCall` block, a `toolResult`/`user` role or an empty-content assistant record (the placeholder pi appends when a reply starts) all mean mid-turn, text blocks mean finished; pi records no error or abort signal. A mid-turn tail counts as `active` only when `session.timestamp` is within `ACTIVE_WINDOW_S` (120s), the Claude pid registry lists it, or the Devin `session_locks` pid is alive; otherwise `interrupted`. Cursor, Codex, OpenCode and Pi keep no per-session process registry. `followup` is a regex ported from Claude Code's own `claude agents` needs-input heuristic over the final assistant text, deliberately under-matching.
- **Rate limits** (`harness/rate_limits.py`): pluggable fetcher architecture. Each tool registers a fetch function returning `RateLimitInfo` (used_percent, limit_reached, reset_at, plan_type). `get_all_rate_limits()` aggregates across fetchers with a 300s disk cache (`~/.quiver/cache/rate_limits_cache.json`, TTL overridable via `SWE_RATE_LIMITS_TTL`). Only starred harnesses are polled. Registered fetchers: Codex (ChatGPT `backend-api/wham/usage` with OAuth tokens from `~/.codex/auth.json`), GitHub Copilot (`api.github.com/copilot_internal/user` with `gh auth token`), Claude, Droid, Freebuff, Cursor (dashboard `get-current-period-usage` with the editor's or CLI's session JWT), Devin (Windsurf's `SeatManagementService/GetUserStatus` with the key from `~/.local/share/devin/credentials.toml`). Add new fetchers with `register(tool_name, fetcher_fn)`.
