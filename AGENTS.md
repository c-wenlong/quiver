...
- **Session status** (`sessions/status.py`): the `swe session` `ST` column resolves one label per row (`active`, `done`, `followup`, `error`, `interrupted`, or "" for harnesses with no probe) and `sessions/commands.py::_STATUS_GLYPHS` maps each to a single-width glyph, colour and legend label; real emoji are two columns wide and `console.visible_len` counts code points, so they would break the grid. Four per-harness signals feed it. Claude: the transcript tail's decisive record, plus the `~/.claude/sessions/*.json` live-pid registry and `~/.claude/jobs/*/state.json` (blocked → followup, failed → error). Cursor: the terminal `turn_ended` record (`success`, `error`, `aborted`). Devin: the `main_chain_id` head row of `message_nodes` (node_id is not unique across sessions, always filter by session_id). Codex: the rollout tail's decisive `event_msg` (`task_complete` → finished with `last_agent_message`, `turn_aborted` → aborted, `task_started` → midturn; the rest are bookkeeping), falling back to the last `response_item` when no decisive event is in the window; the rollout file is found at `~/.codex/sessions/Y/M/D/<stem>.jsonl` by parsing the date out of the stem, with a `*/*/*/` glob as fallback, and codex records no error signal. A mid-turn tail counts as `active` only when `session.timestamp` is within `ACTIVE_WINDOW_S` (120s) or, for Claude, its pid is live; otherwise `interrupted`. `followup` is a regex ported from Claude Code's own `claude agents` needs-input heuristic over the final assistant text, deliberately under-matching.
