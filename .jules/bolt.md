...
## 2024-08-02 - Flat file matching optimization
**Learning:** `glob.glob` followed by `os.path.isfile` causes redundant `stat` syscalls.
**Action:** For flat file matching (patterns without `**` or `/`), prefer using `os.scandir` combined with `str.endswith()` over `glob.glob` followed by `os.path.isfile` to prevent redundant stat syscalls and I/O overhead.
