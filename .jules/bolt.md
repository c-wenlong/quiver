...
## 2024-07-31 - Over-inlining helper functions during refactor
**Learning:** Replacing an existing helper function (like `get_mtime`) directly with native inline calls (e.g. `entry.stat().st_mtime * 1000`) during an optimization (like switching to `os.scandir`) is a risky assumption. It drops fallback logic, exception handling, and potentially domain-specific behavior hidden inside the helper.
