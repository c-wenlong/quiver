...
## 2026-08-07 - Avoid flat glob.glob in favor of os.scandir
**Learning:** `glob.glob` followed by `os.path.isfile` causes unnecessary overhead and redundant stat syscalls, slowing down file scanning.
**Action:** For flat file matching (patterns without `**` or `/`), use `os.scandir` combined with `str.endswith()` or `fnmatch.fnmatch` to prevent redundant stat syscalls and I/O overhead.
