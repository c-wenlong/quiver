...
## 2026-10-24 - Flat Glob Performance Overhead
**Learning:** For flat file matching (patterns without `**` or `/`), `glob.glob` followed by `os.path.isfile` causes redundant stat syscalls and I/O overhead.
**Action:** Prefer using `os.scandir` combined with `str.endswith()` or `fnmatch.fnmatch` over `glob.glob` followed by `os.path.isfile` to prevent redundant stat syscalls and improve I/O performance during directory scanning.
