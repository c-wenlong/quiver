...
## 2024-08-05 - Avoid flat globs for performance
**Learning:** `glob.glob("path/*.ext")` incurs redundant stat syscalls when we just need file extension matching.
**Action:** Prefer `os.scandir` combined with `.is_file()` and `.endswith()` for simple directory-level flat filtering.
