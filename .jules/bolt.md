...
## 2024-08-05 - Flat File Matching Overhead with glob.glob
**Learning:** Using `glob.glob` for flat file matching (without `**` or `/` patterns), particularly when followed by iterating or checking the files, introduces redundant `stat` syscalls and significant I/O overhead.
**Action:** Prefer `os.scandir` combined with `str.endswith()` over `glob.glob` for flat file matching to yield `DirEntry` objects with cached attributes, avoiding unnecessary file system overhead.
