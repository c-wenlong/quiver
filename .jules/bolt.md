...
## 2024-07-25 - Avoid `glob.glob` for shallow file listing
**Learning:** `glob.glob` combined with `os.path.join` performs slower than iterating through `os.scandir` when doing shallow file listings and filtering by extension, as it invokes `fnmatch` string matching.
**Action:** Use `os.scandir` combined with `e.is_file()` and `e.name.endswith()` for shallow file filtering to reduce redundant stat calls and I/O overhead. Ensure it is wrapped in `try...except` to prevent application crashes from inaccessible files.
